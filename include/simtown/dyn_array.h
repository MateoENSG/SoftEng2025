...
template < class T >
DynArray< T >::DynArray() : size_( 0 ), capacity_( 2 )
{
...
DynArray< T >::DynArray( unsigned int capacity )
    : size_( capacity ), capacity_{ ( capacity >= 1 ) ? capacity : 2 }
{
    std::cout << "DynArray::DynArray()" << std::endl;
    // assert( size_ >= 0);
    elements_ = new T[capacity_];
...
template < class T >
unsigned int DynArray< T >::push_back( const T& item )
{
...
template < class T >
const T& DynArray< T >::operator[]( unsigned int i ) const
{
...
{
    std::cout << "DynArray::operator[]" << std::endl;
    // assert(i > -1); // on a tout mis en unsigned int donc forcement entier
    // positif.
    assert( i < size_ );
    return elements_[i];
}
...
DynArray< T >& DynArray< T >::operator=( const DynArray< T >& rhs )
{
    if( this != &rhs )
    {
        ...
        elements_ =
            new T[capacity_]; // new T[size_]; fonctionnerai aussi et
                              // minimiserai l'usage mémoire. Mais un clonage
                              // parfait peut éviter certaines erreurs !
        for( unsigned int i = 0; i < rhs.size_; ++i )
        {
            elements_[i] = rhs.elements_[i];
        }

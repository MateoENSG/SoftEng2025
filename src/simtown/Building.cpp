...
Building::Building( int id, unsigned int nb_rocks )
    : id_( id ), rocks_( nb_rocks )
{
    std::cout << "Building::Building(" << id << ") Created at " << this
              << std::endl;
...
Building::~Building()
{
    ...
    std::cout << "Reappropriation de vos biens par le Gouvernement Celeste de "
                 "la Fronce !!!"
              << std::endl;
    for( unsigned int ir = 0; ir < rocks_.size(); ++ir )
    {
...
{
    std::cout << "Building::Building(Building&), id_= " << id_ << " at " << this
              << std::endl;
...
{
    std::cout << "Building::operator=(" << &rhs << ", " << rhs.id_
...
{
    std::cout << rocks_[index]
              << std::endl; // test affichage pour savoir s'il le trouve bien
    return rocks_[index];
}

void Building::swap_rock(
    Building partner, unsigned int i, unsigned int i_partner )
{
